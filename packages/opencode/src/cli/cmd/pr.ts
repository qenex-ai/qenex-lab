...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run qenex lab",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for qenex lab session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found QENEX LAB session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`qenex import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting QENEX LAB...")
        UI.println()

        // Launch qenex lab TUI with session ID if available
        const { spawn } = await import("child_process")
        const qenexArgs = sessionId ? ["-s", sessionId] : []
        const qenexProcess = spawn("qenex", qenexArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          qenexProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`qenex exited with code ${code}`))
          })
          qenexProcess.on("error", reject)
        })
      },
    })
  },

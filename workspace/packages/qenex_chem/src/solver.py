...
class HartreeFockSolver:
    ...
    def compute_energy(
        ...
        inv_sqrt_evals = np.array(
            [1.0 / np.sqrt(e) if e > 1e-6 else 0.0 for e in evals]
        )
        # X = evecs @ diag @ evecs.T - use PROMETHEUS if available
        ...
        for iteration in range(max_iter):
            ...
            if verbose:
                print(f"Iter {iteration}: E = {curr_E:.8f} (diff {diff:.2e})")
...
class UHFSolver(HartreeFockSolver):
    ...
    def compute_energy(
        ...
        # Initial Guess (Core Hamiltonian) - use PROMETHEUS if available
        if PROMETHEUS_AVAILABLE:
            F_0_prime = transform_fock(X, H_core)
        else:
            F_0_prime = X.T @ H_core @ X
        ...
        # Build initial densities - use PROMETHEUS DGEMM if available
        if PROMETHEUS_AVAILABLE:
            P_alpha, P_beta = build_density_matrix_uhf(C_alpha, C_beta, n_alpha, n_beta)
        else:
    ...
    def compute_gradient(self, molecule: Molecule):
        ...
        for mu in range(len(basis)):
            for nu in range(len(basis)):
                for i in range(n_a):
                    W[mu, nu] += (
                        self.eps_alpha[i] * self.C_alpha[mu, i] * self.C_alpha[nu, i]
                    )
                for i in range(n_b):
                    W[mu, nu] += (
                        self.eps_beta[i] * self.C_beta[mu, i] * self.C_beta[nu, i]
                    )
...
class MP2Solver:
    ...
    def _transform_eri_to_mo(
        ...
        # Check for PROMETHEUS acceleration
        try:
            import qenex_accelerate

            use_prometheus = hasattr(qenex_accelerate, "prometheus_dgemm")
        except ImportError:
    ...
    def _transform_eri_to_mo_accelerated(self, ERI_ao, C_occ, C_vir, verbose=True):
        ...
        if verbose:
            print("  Step 2/4: Second quarter transform (ν -> a)...")
        tmp1_perm = tmp1.transpose(1, 0, 2, 3).reshape(N, -1)
        ...
        if verbose:
            print("  Step 3/4: Third quarter transform (λ -> j)...")
        tmp2_perm = tmp2.transpose(2, 0, 1, 3).reshape(N, -1)
        ...
        if verbose:
            print("  Step 4/4: Fourth quarter transform (σ -> b)...")
        tmp3_perm = tmp3.transpose(3, 0, 1, 2).reshape(N, -1)

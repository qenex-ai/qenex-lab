...
def _init_backend() -> None:
    ...
    # Try PyO3 bindings first (lowest overhead)
    try:
        import qenex_accelerate  # type: ignore[import-not-found]

        if qenex_accelerate.prometheus_is_available():
            _backend = "pyo3"
            _prometheus_available = True
            _pyo3_module = qenex_accelerate
            print("[PROMETHEUS] PyO3 backend loaded (qenex_accelerate)")
            return
    except ImportError:
        pass

    # Fall back to ctypes bindings
    try:
        import sys
        ...
        if is_available():
            ...
            print("[PROMETHEUS] ctypes backend loaded")
            return
    except ImportError:
        pass

    # Check environment variable path as last resort
    lib_path = os.environ.get("PROMETHEUS_LIB_PATH")
    if lib_path:
        try:
            import ctypes
    ...
    print("[PROMETHEUS] Backend not available, using NumPy fallback")
...
def matmul(
    ...
    if _backend == "pyo3" and _pyo3_module is not None:
        return _pyo3_module.prometheus_dgemm(A, B, alpha, beta)
    elif _backend == "ctypes" and _ctypes_dgemm_func is not None:
        return _ctypes_dgemm_func(A, B, alpha=alpha, beta=beta, C=C)
    else:
        if C is None:
            return alpha * (A @ B)
        else:
